...
export const identityResourceClaim = {
  ...
  form: {
    ...
    reset: (target: IIdentityResourceClaim) => {
      Object.assign(target, identityResourceClaim.form.create(target.identityResourceId));
    },
  ...
  async get(id: string): Promise<IIdentityResourceClaim> {
    const response = await http.get<IIdentityResourceClaim>(`/api/identity-resource-claim/get/${id}`);
    return response.data;
  },

  async getByIdentityResourceId(identityResourceId: string): Promise<IIdentityResourceClaim[]> {
    const response = await http.get<IIdentityResourceClaim[]>(`/api/identity-resource-claim/get-by-resource/${identityResourceId}`);
    return response.data;
  },
  ...
  async create(model: IIdentityResourceClaim): Promise<number> {
    const response = await http.post<number>('/api/identity-resource-claim/insert', model);
    return response.data;
  },

  async update(model: IIdentityResourceClaim): Promise<boolean> {
    const response = await http.put<boolean>('/api/identity-resource-claim/update', model);
    return response.data;
  },

  async delete(id: string): Promise<boolean> {
    const response = await http.delete<boolean>(`/api/identity-resource-claim/delete/${id}`);
    return response.data;
  },

  async deleteByIdentityResourceId(identityResourceId: string): Promise<boolean> {
    const response = await http.delete<boolean>(`/api/identity-resource-claim/delete-by-resource/${identityResourceId}`);
    return response.data;
  }
};