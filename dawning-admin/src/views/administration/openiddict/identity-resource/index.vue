<template>
  <div class="container">
    <Breadcrumb :items="['认证授权', '身份资源管理']" />
    <a-card class="general-card" title="身份资源管理">
      <!-- 搜索区 -->
    ...
    >
      <a-form :model="formData" ref="formRef" layout="vertical">
        ...
        <a-form-item field="userClaims" label="声明类型">
          <a-input-tag v-model="formData.userClaims" placeholder="输入后按回车添加" allow-clear />
        </a-form-item>
      </a-form>
    </a-modal>
  </div>
...
import { identityResourceApi, IIdentityResource, IIdentityResourceModel } from '@/api/openiddict/identity-resource-api';
...
async function loadData() {
  loading.value = true;
  try {
    const res = await identityResourceApi.getPagedList(searchForm, pageIndex.value, pageSize.value);
